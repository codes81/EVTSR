    ...
    if quantize:
        ...
        with open(flow_path, rb) as f:
            try:
                ...
                raise IOError(fInvalid flow file: {flow_path})
            else:
                if header != PIEH:
                    raise IOError(fInvalid flow file: {flow_path}, header does not contain PIEH)
...
def quantize(arr, min_val, max_val, levels, dtype=np.int64):
    ...
    if not (isinstance(levels, int) and levels > 1):
        raise ValueError(flevels must be a positive integer, but got {levels})
    if min_val >= max_val:
        raise ValueError(fmin_val ({min_val}) must be smaller than max_val ({max_val}))
